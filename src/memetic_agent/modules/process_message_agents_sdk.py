...
from agents import Agent as OpenAIAgent # renaming as Agent is already defined in the base_agent.type module
...
async def process_message_impl(agent: Agent, content: str, sender: str, conversation_id: str) -> str:
    ...
    try:
        ...
        agent_input = agent.conversations[conversation_id]
        instructions = agent.prompt.system.content

        # Add user message if not duplicate
        last_message = agent_input[-1] if agent_input else None
        if not last_message or last_message.content != content:
            ...
            agent_input.append(user_message)
        ...
        while iteration_count < max_iterations:
            try:
                ...
                response = await asyncio.wait_for(
                    agent.runner.run(agentsdk, inputs=[i.dict() for i in agent_input]),
                    timeout=3050
                )
